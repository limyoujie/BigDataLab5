...
Reducer<TimeseriesKey, TimeseriesDataPoint, Text, Text> {
	...
	throws IOException {
	    ...
	    int iWindowSizeInDays_100 = this.configuration.getInt(
		    "tv.floe.caduceus.hadoop.movingaverage.windowSize", 3);
	    int iWindowStepSizeInDays_100 = this.configuration.getInt(
		    "tv.floe.caduceus.hadoop.movingaverage.windowStepSize", 1);
	    ...
	    int iWindowSizeInDays_200 = this.configuration.getInt(
		    "tv.floe.caduceus.hadoop.movingaverage.windowSize", 4);
	    int iWindowStepSizeInDays_200 = this.configuration.getInt(
		    "tv.floe.caduceus.hadoop.movingaverage.windowStepSize", 1);

	    long iWindowSizeInMS_200 = 4 * day_in_ms;
	    long iWindowStepSizeInMS_200 = 1 * day_in_ms;

	    SlidingWindow sliding_window_200 = new SlidingWindow(iWindowSizeInMS_200,
		    iWindowStepSizeInMS_200, day_in_ms);
		...
		while ((sliding_window_200.WindowIsFull()==false|| sliding_window_100.WindowIsFull() == false || sliding_window.WindowIsFull() == false) && values.hasNext()) {
